    ...
    public function register(Container $app) : void
    {
            ...
            return new ExceptionLoggingCache(
                new CompressingFlightCache(
                    new DoctrineFlightCache(
                        ...
                        $config->remarks->flight_cache->life_time
                    )
                ),
                $app['monolog']
            );
        ...
        $app['cache.flights.memcache'] = function () use ($app, $config) {
            ...
            $memcachedCache->getMemcached()->addServer(
                $config->remarks->flight_cache->memcache->host,
                $config->remarks->flight_cache->memcache->port
            );
        ...
        $app['cache.flights.redis'] = function () use ($app, $config) {
            $redis = new \Redis();
            $redis->connect(
                $config->remarks->flight_cache->redis->host,
                $config->remarks->flight_cache->redis->port,
                1.0
            );
        ...
        $app['cache.flights'] = function () use ($app, $config) {
            $cacheName = "cache.flights.{$config->remarks->flight_cache->adapter}";
